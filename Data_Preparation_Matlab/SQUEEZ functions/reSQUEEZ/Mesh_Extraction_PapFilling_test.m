function [Mesh, info] = Mesh_Extraction_PapFilling_test(info)
...
for j = 1:length(info.timeframes) % Time frame loop
    ...
    if info.fill_paps
        ...
        s = regionprops3(temp,'BoundingBox','ConvexImage','PrincipalAxisLength','EigenValues','EigenVectors','Centroid');
        
        val = cell2mat(s.EigenValues);
        [~,idm] = max(val);
        
        vec = cell2mat(s.EigenVectors);
        
        ax = vec(:,idm);
        lp = s.PrincipalAxisLength(idm);
        ...
        di=8;
        convexhull2 = imerode(convexhull,strel('sphere',di));
        
        img_subtract = convexhull2 - temp;
        ...
        for d = 1:length(idx)
            t2 = zeros(size(temp));
            t2(CC.PixelIdxList{idx(d)})=1;
            stats = regionprops3(t2,'Volume','PrincipalAxisLength','EigenValues','EigenVectors');
            vol(d) = stats.Volume;
            vol(d) = vol(d).*info.desired_res*0.001;
            
            %subplot(2,3,d)
            vale = cell2mat(stats.EigenValues);
            [~,idm] = max(vale);
            ax_l = stats.PrincipalAxisLength(idm);
            eigs1 = cell2mat(stats.EigenVectors);
            eigs1 = eigs1(:,idm);
            
            %             quiver3(0,0,0,eigs1(1),eigs1(2),eigs1(3),ax_l);
            %             hold on; quiver3(0,0,0,ax(1),ax(2),ax(3),lp);
            
            dp(d) = dot(eigs1,ax);
            clear t2 eigs ax_l stats
        end
        
        figure;
        subplot(1,2,1); plot(vol,'o'); grid on; grid minor; hold on;
        plot(1:length(idx),ones(1,length(idx))*10,'r'); ylim([0,200])
        subplot(1,2,2); plot(dp,'o'); ylim([-1 1]); grid on; grid minor
        hold on; plot(1:length(idx),ones(1,length(idx))*0.8,'r'); plot(1:length(idx),ones(1,length(idx))*-0.8,'r');
