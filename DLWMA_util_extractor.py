from keras.preprocessing import image # need keras==2.2.5
from keras import applications
#from keras.applications.inception_v3 import InceptionV3, preprocess_input
from keras.models import Model, load_model
from keras.layers import Input
import numpy as np

class Extractor():
    def __init__(self, architecture,weights=None):
        """Either load pretrained from imagenet, or load our saved
        weights from our own training."""
        self.architecture = architecture
        self.weights = weights  # so we can check elsewhere which model

        if weights is None:
            if self.architecture == 'InceptionV3':
                # Get model with pretrained weights.
                base_model = applications.InceptionV3(
                    weights='imagenet',
                    include_top=True
                )

            if self.architecture == 'Xception':
                # Get model with pretrained weights.
                base_model = applications.Xception(
                    weights='imagenet',
                    include_top=True
                )

            if self.architecture == 'ResNet':
                # Get model with pretrained weights.
                base_model = applications.ResNet152V2(
                    weights='imagenet',
                    include_top=True
                )
                  
            if self.architecture == 'InceptionResNetV2':
                # Get model with pretrained weights.
                base_model = applications.InceptionResNetV2(
                    weights='imagenet',
                    include_top=True
                )
            # We'll extract features at the final pool layer.   
            self.model = Model(
                inputs=base_model.input,
                outputs=base_model.get_layer('avg_pool').output
                )
            print(base_model.summary())      
            

        else:
            # Load the model first.
            self.model = load_model(weights)

            # Then remove the top so we get features not predictions.
            # From: https://github.com/fchollet/keras/issues/2371
            self.model.layers.pop()
            self.model.layers.pop()  # two pops to get to pool layer
            self.model.outputs = [self.model.layers[-1].output]
            self.model.output_layers = [self.model.layers[-1]]
            self.model.layers[-1].outbound_nodes = []

    def extract(self, image_path):
        if self.architecture == 'InceptionV3':
            img = image.load_img(image_path, target_size=(299, 299))
            x = image.img_to_array(img)
            x = np.expand_dims(x, axis=0)
            x = applications.inception_v3.preprocess_input(x)
        if self.architecture == 'Xception':
            img = image.load_img(image_path, target_size=(299, 299))
            x = image.img_to_array(img)
            x = np.expand_dims(x, axis=0)
            x = applications.xception.preprocess_input(x)
        if self.architecture == 'ResNet':
            img = image.load_img(image_path, target_size=(224, 224))
            x = image.img_to_array(img)
            x = np.expand_dims(x, axis=0)
            x = applications.resnet_v2.preprocess_input(x)
        if self.architecture == 'InceptionResNetV2':
            img = image.load_img(image_path, target_size=(299,299))
            x = image.img_to_array(img)
            x = np.expand_dims(x, axis=0)
            x = applications.inception_resnet_v2.preprocess_input(x)
        

        # Get the prediction.
        features = self.model.predict(x)
        #print(self.model.summary())

        if self.weights is None:
            # For imagenet/default network:
            features = features[0]
        else:
            # For loaded network:
            features = features[0]

        return features
